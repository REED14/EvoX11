...
//variables for FakeBlur
bool PaintBlur = 0;
cairo_surface_t* png_blur;
Drawable pb_drw;
int pb_wid, pb_hei, pb_depth;
Picture pb_pic = 0;
Visual* pb_vis;
char pb_addr[1000];
...
paint_all (Display *dpy, XserverRegion region)
{
	...
	if(w->delMaskPict == 1 && w->maskPict!=None){
		XRenderFreePicture(dpy, w->maskPict);
		w->maskPict = None;
		Picture sw = solid_picture (dpy, False, (double) w->opacity / OPAQUE, 0, 0, 0);
		w->maskPict = RoundedCorners(dpy, wid, hei, root, sw, GlobalRadius*(hei>GlobalRadius*3));
		XRenderFreePicture(dpy, sw);
		w->delMaskPict = 0;
	}

		if(get_time_in_milliseconds()-w->map_time<250 &&
		   get_time_in_milliseconds()-w->map_time>0){

			double delta = (get_time_in_milliseconds()-w->map_time)/250.0;

			XTransform xwin = {{
				{ XDoubleToFixed( delta ), XDoubleToFixed( 0 ), XDoubleToFixed( 0 ) },
				{ XDoubleToFixed( 0 ), XDoubleToFixed( 1 ), XDoubleToFixed( -hei/2*(1-delta) ) },
				{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( delta ) }
			}};
			
			XRenderSetPictureTransform( dpy, w->picture, &xwin );
		}

		else if(get_time_in_milliseconds()-w->unmap_time<1000 &&
		   		get_time_in_milliseconds()-w->unmap_time>0){

			double delta = (get_time_in_milliseconds()-w->unmap_time)/258.0;

			XTransform xwin = {{
				{ XDoubleToFixed( 1 ), XDoubleToFixed( 0 ), XDoubleToFixed( -wid/2*delta ) },
				{ XDoubleToFixed( 0 ), XDoubleToFixed( 1-delta ), XDoubleToFixed( 0 ) },
				{ XDoubleToFixed( 0 ), XDoubleToFixed( 0 ), XDoubleToFixed( 1-delta ) }
			}};
	...
	case CompServerShadows:
	    ...
	    if (w->windowType == winDesktopAtom || w==c)
		break;
	    set_ignore (dpy, NextRequest (dpy));
	...
	case CompClientShadows:
	    ...
	    if (w->shadow && w->windowType != winDesktopAtom && w!=c)
	    {
		set_ignore (dpy, NextRequest (dpy));
	...
	else if (w->mode == WINDOW_ARGB)
	{
		...
		if(ConfigWin==w->id && get_time_in_milliseconds()-time_conf<200)
			...
			if(PaintBlur && pb_pic && w->windowType!=winDockAtom && !in_trans && w->maskPict!=None){
				Picture sw = solid_picture (dpy, False, 1, 0, 0, 0);
				if(w->bkgMaskPict == None) w->bkgMaskPict = BkgRoundedCorners(dpy, wid, hei, root, sw, GlobalRadius*(hei>GlobalRadius*3));
			...
			if(wid<root_width-30 && hei<root_height-30 && w->maskPict!=None)
				XRenderComposite (dpy, PictOpOver, w->picture, w->maskPict, rootBuffer,
						0, 0, 0, 0,
						x, y, wid, hei);
			else
				XRenderComposite (dpy, PictOpOver, w->picture, None, rootBuffer,
	...
	if(c!=NULL){
			set_ignore (dpy, NextRequest (dpy));
			CompConf(dpy, c->picture, rootBuffer, c->a.x, c->a.y, c->a.width, c->a.height, 
			get_time_in_milliseconds(), root, root_width, root_height);
	}
...
map_win (Display *dpy, Window id, unsigned long sequence, Bool doFade)
{
	...
	//sets time of mapping and disables the unmap animation
	w->map_time = get_time_in_milliseconds();
	w->unmap_time = 0;
...
unmap_win (Display *dpy, Window id, Bool doFade)
{
	...
	//sets time of unmapping and disables the map animation
	w->unmap_time = get_time_in_milliseconds();
	w->map_time = 0;
...
configure_win (Display *dpy, XConfigureEvent *ce)
{
	...
	conf_depth_x = ce->x - w->a.x;
	conf_depth_y = ce->y - w->a.y;

	if(ce->width!=w->a.width || ce->height!=w->a.height){
		conf_depth_x = 0;
		conf_depth_y = 0;
	}
...
register_cm (Display *dpy)
{
    ...
    Xutf8SetWMProperties (dpy, w, "EvoX11", "EvoX11", NULL, 0, NULL, NULL,
			  NULL);
...
main (int argc, char **argv)
{
	...
	//check if fake blur png exists
	PaintBlur = 0;
	if(pb_addr[0]!='\0')
	if(file_exists(pb_addr))
		PaintBlur = 1;

	//get the fake blur png
	
	if(PaintBlur){
		png_blur = cairo_image_surface_create_from_png(pb_addr);
		cairo_pattern_t *plpt = cairo_pattern_create_for_surface(png_blur);

		//pb_drw = cairo_xlib_surface_get_drawable(png_blur);
		pb_wid = cairo_xlib_surface_get_width(png_blur);
		pb_hei = cairo_xlib_surface_get_height(png_blur);
		pb_depth = cairo_xlib_surface_get_depth(png_blur);
		pb_vis = cairo_xlib_surface_get_visual(png_blur);

		Pixmap pb_pix = XCreatePixmap(dpy, root, root_width, root_height, DefaultDepth(dpy, scr));
		cairo_surface_t* pb_surf = cairo_xlib_surface_create(dpy, pb_pix, DefaultVisual(dpy, scr), 
															root_width+10, root_height+10);
		cairo_t *dest_cr = cairo_create(pb_surf);

		cairo_set_source(dest_cr, plpt);
		//cairo_set_source_rgb(dest_cr, 0.2, 0.4, 0.1);
    	cairo_paint(dest_cr);

		pb_pic = XRenderCreatePicture(dpy, pb_pix, 
	...
	do {
		...
		if(ev.type == ConfigureNotify){
			...
			if(get_time_in_milliseconds()-time_conf>200 || 
			   ConfigWin!=ev.xconfigure.window){
			   distort_x = 0;
			   distort_y = 0;
			}

			ConfigWin = ev.xconfigure.window;
			time_conf = get_time_in_milliseconds();
		}
		else{
			is_conf = 0;
		}
